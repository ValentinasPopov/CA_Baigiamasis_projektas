...
def train(dataloader, model, optimizer, criterion, epochs, device, target_accuracy=None):
    ...
    """
    Loss - it is a summation of the errors made for each sample in training or validation sets
    Accuracy - is a method for measuring a classification model's performace. 
    Accuracy is the count of predictions where the predicted value is equal to the true value.
    Weights - a learnable parameter of some machine learning models, including neural networks.
    """
...
def get_bbox_from_heatmap(heatmap, thres=0.8):
    ...
    y_0 = int(y_dim[x_dim > 0].min())
    y_1 = int(y_dim.max())

    return x_0, y_0, x_1, y_1


def predict_localize(model, dataloader, device, thres, n_samples=9, show_heatmap=False):
    """
